...
public class Energizer implements Collidable{
    ...
    public Energizer(Pane gamePane, int i, int j, Game game) {
        ...
        game.getEnergizerArrayList().add(_energizer);
        gamePane.getChildren().add(_energizer);
    }

    public int collide(Pacman pacman, Pane gamePane, Game game) {
        if(!game.getEnergizerArrayList().isEmpty()) {
            game.getEnergizerArrayList().remove(_energizer);
        }
